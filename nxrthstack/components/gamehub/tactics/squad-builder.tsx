...
export function SquadBuilder({ attackSquad, defenseSquad, unlockedUnitIds, onSave }: SquadBuilderProps) {
      ...
      <div className="flex gap-2">
        {(["attack", "defense"] as const).map((tab) => (
          <button
            key={tab}
            type="button"
            onClick={() => { setActiveTab(tab); setSelectedUnitIndex(null); setEditingBehavior(null); }}
            className={cn(
